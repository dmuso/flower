...
* For code and test changes, always run the relevant tests and linting after a change to verify your work. For documentation-only changes, use documentation-appropriate validation such as reviewing the diff and running `git diff --check` for the changed files; do not run code linting or test suites unless code or test files changed.
...
- **Testing**: For code and test changes, always run `nix-shell --run 'make test'` and `nix-shell --run 'make lint'` after making changes. For documentation-only changes, validate the documentation diff instead and do not run the code gates unless code or test files changed.
