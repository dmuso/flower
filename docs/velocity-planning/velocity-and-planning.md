...
| **Velocity** | Live rate: corpus estimate-points per unit time. Undefined when the corpus is empty or `time == 0`. |
| **Predicted duration** | How long an incomplete story of a given estimate is expected to take, from completed corpus stories of the same estimate. Written `predicted_duration(estimate)`. |
...
- Window `i` (i = 0, 1, 2, …) is the half-open range:
  ...
  - `ends_at(i) = starts_on(i) + iteration_length_days` at `00:00` project timezone (midnight at the start of the next window)
  - Last calendar day of the window = `ends_at(i) − 1 day`. Display as `Ends {that date}`.
- **Current window** = the unique `i` where `starts_on(i) ≤ now < ends_at(i)` in project timezone.
- When `now` crosses `ends_at` of the open window, that window is **completed**. Accepted work whose `accepted_at` fell in it ages into Done. `velocity` and `predicted_duration` recompute from stories. Remaining unstarted stories are re-packed. In-flight stay in Current. Unaccepted work is not failed and not iceboxed.
...
```
duration = accepted_at − started_at
```

Reject / Restart does **not** reset `started_at`. The clock is first start.

Size key: the story's **current** estimate (live). Edits to estimate move the story's `duration` into the new size bucket on the next recompute.

Lookback: corpus stories whose `accepted_at` falls in the last number of completed windows set by `velocity_strategy`. `velocity_strategy` default **3**, allowed **1–4** (project setting). If fewer completed windows exist, use what exists. Stories accepted in the **open** window are not in the lookback.
...
time = sum(duration) of corpus stories

if time == 0 or corpus is empty:
    velocity is undefined          # cold start
else:
    velocity = work / time         # estimate-points per unit time
```

Compute `duration` from timestamps in a consistent unit (fractional days). `velocity` is then estimate-points per day. `iteration_length_days` and time remaining use the same unit.

Window capacity in estimate-points:

```
capacity           = velocity × iteration_length_days   # a full window of that length in days
capacity_remaining = velocity × time_remaining_in_window  # open window: now → ends_at
```

Do not kick out in-flight or accepted-this-window stories, even when they make Current exceed capacity.

`velocity` is a calculation. It is not persisted. Deleting a corpus story, changing its estimate, or editing timestamps (via undo) recomputes `velocity` and `predicted_duration` from the stories that remain.
...
```
if the corpus has at least one story whose estimate matches:
    predicted_duration(estimate) = mean(duration) of those stories
else if velocity is defined:
    predicted_duration(estimate) = estimate / velocity
else:
    # cold start — pack does not use predicted_duration; see Cold start
```

Unestimated Features: cost / duration **0** for packing fill rules. They still **cannot Start**. They do not contribute to velocity.

Bugs, chores, and releases: duration **0** for packing unless the bugs-and-chores-points toggle is on and the bug/chore has an estimate — then `predicted_duration(estimate)` applies as above.
...
- `velocity` — live rate, or undefined
- `predicted_duration` — the size → predicted-duration map (unused during cold start)
- `iteration_length_days`, `now`, `timezone` — window clock
...
**Duration model** (`velocity` defined):

```
duration(story) =
  predicted_duration(estimate)  if the story has an estimate and is a Feature
                                (or a bug/chore with the points toggle on)
  0                             otherwise   # unestimated, bugs, chores, releases
```

**Cold start** (`velocity` undefined):
...
2. Remaining capacity in Current = time from `now` to `ends_at` (the remaining predicted-duration budget). Equivalent point view: `capacity_remaining = velocity × time_remaining_in_window`.
3. Walk the remaining `unstarted` / Release-`started` stories in rank order:
   - A story's predicted duration is `predicted_duration(estimate)` (or 0 if unestimated / non-pointed).
   - If `duration > remaining time`, **do not place it in this band**. Leave the band **short**. Open the next band with a full window of `iteration_length_days` and try again.
...
Same walk, but the budget is `initial_velocity` estimate-points in each full window (including Current). Leave short rather than overfill. Do not use `predicted_duration` or `velocity`.
...
Off by default. When off for Current: Current contains only in-flight, accepted-this-window, and stories **explicitly moved** there. No velocity fill into Current. Backlog future bands still auto-plan (duration model, or bootstrap points if `velocity` is undefined). Restoring auto-plan reruns `pack`.

## Re-plan is live

Recompute `pack` on: create, delete, icebox, schedule, reorder, estimate change, type change, start (and other verbs), accept, reject, restart, undo, window end, length / timezone / strategy / initial-velocity / bugs-and-chores toggle change.

No Recalculate button. The board after a mutation is already right.

## Window end

Clock crossing only. At `ends_at` of the open window (midnight, project timezone):
...
3. Those accepted Features (with `started_at` and `accepted_at`) now sit in a completed window, so they join the lookback corpus if they fall in the last number of completed windows set by `velocity_strategy`.
4. `velocity` and `predicted_duration` recompute from stories.
...
Display the date only, project timezone. Icebox: “Not scheduled.”
...
- Bar per completed window. Line at current `velocity` converted to a full-window capacity (`velocity × iteration_length_days`), or at `initial_velocity` while the corpus is empty.
...
- Empty: “No scope yet.”
- Cycle time (Phase 3) is the same clock as `duration`: first `started_at` → `accepted_at`.
...
Project created Thursday 6 Aug 2026. Timezone `Australia/Melbourne`. Length **7 days**, starts Monday. First start = Monday 3 Aug. Current window: Mon 3 Aug 00:00 – Mon 10 Aug 00:00. Displayed end: **9 Aug**. Corpus is empty. `velocity` is undefined. Pack uses `initial_velocity` **10** estimate-points per full window.

`pack(ordered_stories, velocity=undefined, predicted_duration=empty, iteration_length_days=7, now=Thu 6 Aug, timezone=Australia/Melbourne)`:
...
They Start Login Thursday 6 Aug 09:00 and Accept it Friday 7 Aug 09:00. `started_at` = Thu 09:00, `accepted_at` = Fri 09:00, `duration` = **1 day**, estimate **2**. Login stays in **Current** (accepted-this-window). Still not Done. The open window is not in the lookback, so `velocity` is still undefined and pack stays on bootstrap.

Monday 10 Aug 00:00 Melbourne: current window ends.

- Login → Done (flat list; newest accepted first). No row is written.
- Corpus now has Login (`accepted_at` in the completed 3–10 Aug window). `work = 2`, `time = 1 day`, **`velocity = 2` points/day**.
- `predicted_duration(2) = 1 day` (mean of Login). No other size in the corpus, so `predicted_duration(1) = 1/2 day`, `predicted_duration(3) = 1.5 days`, `predicted_duration(5) = 2.5 days`.
- In-flight: Billing started. Stays in Current.
- Remaining time in the new window (10–17 Aug) = **7 days**.
- Reset `predicted_duration(1) = 0.5d` → Current (6.5d left). Search `predicted_duration(3) = 1.5d` → Current (5d). Admin `predicted_duration(3) = 1.5d` → Current (3.5d).
- Current: Billing (started) + Reset + Search + Admin. Packed from `predicted_duration` against the 7 days remaining, at `velocity = 2` points/day.

## Worked example 2 — similar-size predicted duration, leave-short, release colour vs computed window

Last three completed windows (3–10, 10–17, 17–24 Aug 2026). `velocity_strategy = 3`. Corpus:

| Story | Est | `started_at` | `accepted_at` | `duration` |
| --- | --- | --- | --- | --- |
...
`work = 17`, `time = 17 days`, **`velocity = 1` point/day**. Full-window capacity = `1 × 7` = **7** points.

```
predicted_duration(1) = mean(1d, 1d) = 1 day
predicted_duration(2) = mean(2d, 2d) = 2 days
predicted_duration(3) = mean(3d, 3d) = 3 days
predicted_duration(5) = mean(5d)     = 5 days
```

No corpus story of estimate 8, so `predicted_duration(8) = 8 / velocity` = 8 days (oversized versus a 7-day window).

Current window starts **24 Aug 2026**, ends at midnight **31 Aug** (displayed end **30 Aug**). `iteration_length_days` = 7. `now` = **Wednesday 26 Aug 00:00**. Time remaining in Current = **5 days**.

| Pos | Story | Type | Est | State | `predicted_duration` |
| --- | --- | --- | --- | --- | --- |
...
Checkout is Delivered in Current after Luis Started it (overflow velocity). Maya Rejects: “tax is wrong.” State `rejected`, still Current. Restart → `started`, still Current. `started_at` is still the first Start. `velocity` and `predicted_duration` are unchanged (Checkout is not accepted).

They Accept Checkout on **2 Sep** (next window). `duration` = first `started_at` → `accepted_at` on 2 Sep. Checkout stays in Current until that window ends, then Done. It joins the corpus when its `accepted_at` sits in a completed lookback window. It does not rewrite an earlier window.
...
4. Advance the test clock past midnight at the window end. Confirm that Feature is in Done (flat list) and `velocity` is `estimate / duration` from that story (`started_at` → `accepted_at`), not a window point sum.
